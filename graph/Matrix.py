class Matrix:
    ...
    def rows(self, string):
        if string == "amount": return len(self.row)
        elif string == "": return self.row
    def columns(self, string):
        if string == "amount": return len(self.row[0])
        elif string == "":
            columns = list()
    ...
    def add(m1, m2):
        ...
        for row in range(len(m1.row)):
            for column in range(len(row)):
                matrix.enter(m1.row[row][column]+m2.row[row][column], row, column)
        return matrix
    def multiply(m1, m2):
        ...
        for i in m2.columns("amount"): # <=> i is a column variable for m2
            for k in m1.rows("amount"): # <=> k is a row variable for m1
                sum = 0
                for j in range(m2.rows("amount")): # or in range(m1.columns) <=> j is a row variabel for m2 and a column variable for m1 
